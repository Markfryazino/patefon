...
def _generate_orbit(point_0, r, n=1000):
    X = np.zeros([n, 2])
    xcur, ycur = point_0[0], point_0[1]

    for idx in range(n):
        xcur = (xcur + r * ycur * (1.0 - ycur)) % 1
        ycur = (ycur + r * xcur * (1.0 - xcur)) % 1
        X[idx, :] = [xcur, ycur]
...
def generate_orbits(m, rs=[2.5, 3.5, 4.0, 4.1, 4.3], n=1000, random_state=None):
    ...
    for j, r in enumerate(rs):
        ...
        for i, point_0 in enumerate(points_0):
            orbits[j * m + i] = _generate_orbit(points_0[i], rs[j])
...
def create_dataset_of_diagrams(objects_per_class=1000):
    X, y = generate_orbits(objects_per_class, random_state=np.random.RandomState(42))
...
def _filter_topk_homology(H0, H1, top0, top1):
...
class OrbitDataset(torch.utils.data.Dataset):
    def __init__(self, diagrams, y, top0, top1):
        ...
        for H0, H1 in tqdm(diagrams):
            self.H.append(_filter_topk_homology(H0, H1, top0, top1))
...
def collate_orbits(data):
    ...
    return {"H0": H0_matrix, "H1": H1_matrix, "H0_mask": H0_mask, "H1_mask": H1_mask, "target": y}
